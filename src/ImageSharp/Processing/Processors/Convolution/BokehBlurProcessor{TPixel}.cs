    ...
    where TPixel : unmanaged, IPixel<TPixel>
{
    ...
    protected override void OnFrameApply(ImageFrame<TPixel> source)
    {
        ...
        if (this.gamma == 3F)
        {
            ApplyGamma3ExposureRowOperation gammaOperation = new ApplyGamma3ExposureRowOperation(sourceRectangle, source.PixelBuffer, this.Configuration);
        ...
        {
            ApplyGammaExposureRowOperation gammaOperation = new ApplyGammaExposureRowOperation(sourceRectangle, source.PixelBuffer, this.Configuration, this.gamma);
        ...
        if (this.gamma == 3F)
        {
            ApplyInverseGamma3ExposureRowOperation operation = new ApplyInverseGamma3ExposureRowOperation(sourceRectangle, source.PixelBuffer, processingBuffer, this.Configuration);
        ...
        {
            ApplyInverseGammaExposureRowOperation operation = new ApplyInverseGammaExposureRowOperation(sourceRectangle, source.PixelBuffer, processingBuffer, this.Configuration, 1 / this.gamma);
            ParallelRowIterator.IterateRows(
                this.Configuration,
                sourceRectangle,
                in operation);
        ...
        Buffer2D<Vector4> processingBuffer)
    {
        ...
        using KernelSamplingMap mapXY = new KernelSamplingMap(configuration.MemoryAllocator);
        ...
        for (int i = 0; i < this.kernels.Length; i++)
        {
            ...
            FirstPassConvolutionRowOperation horizontalOperation = new FirstPassConvolutionRowOperation(
                sourceRectangle,
                firstPassBuffer,
                source.PixelBuffer,
                mapXY,
            ...
            BokehBlurProcessor.SecondPassConvolutionRowOperation verticalOperation = new BokehBlurProcessor.SecondPassConvolutionRowOperation(
                sourceRectangle,
                processingBuffer,
                firstPassBuffer,
                mapXY,
