...
internal sealed class GifDecoderCore : IImageDecoderInternals
{
    ...
    private void ReadFrame<TPixel>(ref Image<TPixel>? image, ref ImageFrame<TPixel>? previousFrame, BufferedReadStream stream)
        where TPixel : unmanaged, IPixel<TPixel>
    {
        this.ReadImageDescriptor(stream);
    ...
    private void ReadFrameIndices(Buffer2D<byte> indices, BufferedReadStream stream)
    {
