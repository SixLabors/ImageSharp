...
internal static class FromLinearStageOperators
{
    /// <summary>
    ...
    {
        /// <summary>
        /// Applies the transfer function on three RGB channels.
        /// </summary>
    ...
    public sealed class RgbOperator : IOperator
    {
        ...
        public void Transform(ref Vector<float> r, ref Vector<float> g, ref Vector<float> b)
        {
            r = JxlSRgbTransferFunction.EncodedFromDisplay(r);
            g = JxlSRgbTransferFunction.EncodedFromDisplay(g);
            b = JxlSRgbTransferFunction.EncodedFromDisplay(b);
        }
    }
    ...
    public sealed class PqOperator(JxlOutputEncodingInfo outputEncodingInfo) : IOperator
    {
        ...
        public void Transform(ref Vector<float> r, ref Vector<float> g, ref Vector<float> b)
        {
            r = this.pq.EncodedFromDisplay(r);
            g = this.pq.EncodedFromDisplay(g);
            b = this.pq.EncodedFromDisplay(b);
        }
    }
    ...
    public sealed class HlgOperator(JxlOutputEncodingInfo outputEncodingInfo) : IOperator
    {
        ...
        private readonly JxlHlgOotfToneMapper hlgOotf = JxlHlgOotfToneMapper.ToSceneLight(
            outputEncodingInfo.DesiredIntensityTarget,
        ...
        public void Transform(ref Vector<float> r, ref Vector<float> g, ref Vector<float> b)
        {
            ...
            r = JxlHlgTransferFunction.EncodedFromDisplay(r);
            g = JxlHlgTransferFunction.EncodedFromDisplay(g);
            b = JxlHlgTransferFunction.EncodedFromDisplay(b);
        }
    }
    ...
    public sealed class Bt709Operator : IOperator
    {
        ...
        public void Transform(ref Vector<float> r, ref Vector<float> g, ref Vector<float> b)
        {
            r = JxlBt709TransferFunction.EncodedFromDisplay(r);
            g = JxlBt709TransferFunction.EncodedFromDisplay(g);
            b = JxlBt709TransferFunction.EncodedFromDisplay(b);
        }
    }
    ...
    {
        /// <summary>
        /// The inverse gamma from output encoding info.
        /// </summary>
        private readonly float inverseGamma = outputEncodingInfo.InverseGamma;
        ...
        public void Transform(ref Vector<float> r, ref Vector<float> g, ref Vector<float> b)
        {
            r = Vector.ConditionalSelect(
                    ...
                    Vector<float>.Zero,
                    JxlSimdUtils.FastPowf(r, Vector.Create(this.inverseGamma)));
            g = Vector.ConditionalSelect(
                    Vector.LessThan(g, Vector.Create(1e-5f)),
                    Vector<float>.Zero,
                    JxlSimdUtils.FastPowf(g, Vector.Create(this.inverseGamma)));
            b = Vector.ConditionalSelect(
                    Vector.LessThan(b, Vector.Create(1e-5f)),
                    Vector<float>.Zero,
                    JxlSimdUtils.FastPowf(b, Vector.Create(this.inverseGamma)));
        }
    }
}
