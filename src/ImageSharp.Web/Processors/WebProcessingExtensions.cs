...
namespace ImageSharp.Web.Processors
{
    ...
    internal static class WebProcessingExtensions
    {
        ...
        public static Image<Rgba32> Process(this Image<Rgba32> source, ref IImageFormat format, ILogger logger, IEnumerable<IImageWebProcessor> processors, IDictionary<string, string> commands)
        {
            foreach (IImageWebProcessor processor in processors)
            {
                source = processor.Process(source, ref format, logger, commands);
            }
