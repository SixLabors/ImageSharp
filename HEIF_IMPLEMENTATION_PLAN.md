...
- [ ] **Active:** eliminate per-call scratch allocation across every AV1 forward and inverse 1-D transform, replace shared mutable transform scratch with operation-owned workspace, and implement libaom-shaped bulk transform paths with scalar parity.
  ...
  - [ ] Port the applicable libaom bulk forward-transform kernels using the existing ImageSharp `Vector128` and `Vector256` infrastructure while retaining the scalar oracle; add a managed `Vector512` path only if complete-block measurements justify extending beyond the pinned upstream SIMD implementations.
