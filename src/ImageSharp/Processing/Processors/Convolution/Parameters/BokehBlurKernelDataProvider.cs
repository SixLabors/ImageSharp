...
internal static class BokehBlurKernelDataProvider
{
        ...
        float kernelsScale)
    {
        ...
        {
            ref Vector4 paramsRef = ref Extensions.UnsafeAdd(ref baseRef, (uint)i);
        ...
        float b)
    {
        ...
        for (int i = 0; i < kernelSize; i++, n++)
        {
            ...
            Extensions.UnsafeAdd(ref baseRef, (uint)i) = new Complex64(
    ...
    private static void NormalizeKernels(Complex64[][] kernels, Vector4[] kernelParameters)
    {
        ...
        for (int i = 0; i < kernelParameters.Length; i++)
        {
            ref Complex64[] kernelRef = ref Extensions.UnsafeAdd(ref baseKernelsRef, (uint)i);
            int length = kernelRef.Length;
            ref Complex64 valueRef = ref Extensions.GetUnsafeArrayDataReference(kernelRef);
            ref Vector4 paramsRef = ref Extensions.UnsafeAdd(ref baseParamsRef, (uint)i);

            for (int j = 0; j < length; j++)
            {
                for (int k = 0; k < length; k++)
                {
                    ref Complex64 jRef = ref Extensions.UnsafeAdd(ref valueRef, (uint)j);
                    ref Complex64 kRef = ref Extensions.UnsafeAdd(ref valueRef, (uint)k);
                    total +=
        ...
        {
            ref Complex64[] kernelsRef = ref Extensions.UnsafeAdd(ref baseKernelsRef, (uint)i);
            int length = kernelsRef.Length;
            ref Complex64 valueRef = ref Extensions.GetUnsafeArrayDataReference(kernelsRef);

            for (int j = 0; j < length; j++)
            {
                Extensions.UnsafeAdd(ref valueRef, (uint)j) *= scalar;
            }
        }
    }
}
