    ...
    where TPixel : unmanaged, IPixel<TPixel>
{
    ...
    public void Invoke(int y, Span<Vector4> span)
    {
        ...
        {
            int currentYIndex = Extensions.UnsafeAdd(ref sampleRowBase, (uint)i);
        ...
        if (this.preserveAlpha)
        {
            for (int x = 0; x < boundsWidth; x++)
            {
                ...
                ref Vector4 target = ref Extensions.UnsafeAdd(ref targetBase, (uint)x);
                for (int kY = 0; kY < state.Kernel.Rows; kY++)
                {
                    ...
                    {
                        int currentXIndex = Extensions.UnsafeAdd(ref sampleColumnBase, (uint)kX) - boundsX;
                        Vector4 pixel = Extensions.UnsafeAdd(ref sourceRowBase, (uint)currentXIndex);
